...
extern int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    ...
    do {
        ...
        /* Aligned, endianness-faithful load: Data points into an arbitrary offset of the
         * fuzz input, so casting it to (unsigned short *) and dereferencing is a misaligned
         * read that halting UBSan aborts on (~half of all inputs) before the parser runs --
         * tanking coverage. memcpy reads the same two LE bytes with no alignment requirement. */
        memcpy(&token_length, Data, sizeof(token_length));
